...
updates_dir=/data/voidui_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have VoidUI-VERSION-DATE-TYPE-* as format"
...
# Assume VoidUI-VERSION-DATE-TYPE-*.zip
...
adb shell "killall org.voidui.updater 2>/dev/null"
adb shell "sqlite3 /data/data/org.voidui.updater/databases/updates.db" \
